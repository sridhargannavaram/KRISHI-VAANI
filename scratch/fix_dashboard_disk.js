...
// 1. Fix descRecAction and Today's Priorities in updateTodayAgriStatus
const recSearch = 'if (descRecAction) {';
const recStartIdx = content.indexOf(recSearch);
const recEndSearch = '// ================= STRUCTURED UI RENDERER FOR SEASONAL CROPS =================';
const recEndIdx = content.indexOf(recEndSearch);

if (recStartIdx === -1 || recEndIdx === -1) {
    console.error('Could not find recAction markers!');
    process.exit(1);
}

const cleanRecAndPriorities = `if (descRecAction) {
...
content = content.substring(0, recStartIdx) + cleanRecAndPriorities + content.substring(recEndIdx);

// 2. Fix fetchDashboardNews
const newsSearch = 'async function fetchDashboardNews() {';
const newsStartIdx = content.indexOf(newsSearch);
const newsEndSearch = '// Hook news loader';
const newsEndIdx = content.indexOf(newsEndSearch);

if (newsStartIdx !== -1 && newsEndIdx !== -1) {
    const cleanNewsJs = `async function fetchDashboardNews() {
            const grid = document.getElementById('dashboardNewsGrid');
            if (!grid) return;
            try {
                const res = await fetch(\`\${API_BASE_URL}/news?category=all\`);
                const data = await res.json();
                const rawArticles = Array.isArray(data) ? data : (Array.isArray(data.articles) ? data.articles : []);
                const articles = rawArticles.slice(0, 3);
                if (articles.length === 0) {
                    grid.innerHTML = '<p style="grid-column: 1/-1; padding: 1rem; color: var(--text-secondary); text-align: center;">No agricultural news available at this time.</p>';
                    return;
                }
        ...
        `;
    content = content.substring(0, newsStartIdx) + cleanNewsJs + content.substring(newsEndIdx);
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully fixed dashboard disk file cleanly!');
